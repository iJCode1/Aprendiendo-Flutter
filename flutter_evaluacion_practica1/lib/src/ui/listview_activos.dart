...
class ListViewActivos extends StatefulWidget {
  ListViewActivos({Key? key}) : super(key: key);
  _ListViewActivosState createState() => _ListViewActivosState();
}

final pastelRF = FirebaseDatabase.instance.reference().child('pastel').orderByChild("status").equalTo("Activo");
final pastelRF2 = FirebaseDatabase.instance.reference().child('pastel');


class _ListViewActivosState extends State<ListViewActivos> {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      title: 'Lista de Pasteles Activos',
      home: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text("Lista de Pasteles Activos"),
  ...
  void _borrarPastel(BuildContext context, Pastel pastel, int position) async {
    await pastelRF2.child(pastel.id.toString()).remove().then((_) {
      setState(() {
